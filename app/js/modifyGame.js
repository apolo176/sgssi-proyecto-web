document.addEventListener("DOMContentLoaded", () => {
    ...
    .then(juego => {
        ...
        form.genero.value = juego.genero;
        form.precioSalida.value = juego.precioSalida;
        form.fechaLanzamiento.value = juego.fechaLanzamiento;
        form.notaMetacritic.value = juego.notaMetacritic;
    });
...
function submitForm(formulario) {
  ...
  datos.forEach((value, key) => {
    ...
    const valorOriginal = original[key] !== undefined ? String(original[key]) : null;
    const valorActual = value !== null ? String(value) : "";

    // Guardamos solo si ha cambiado
    if (valorActual && valorActual !== valorOriginal) {
        cambios[key] = value;
    }
  });
  cambios.id = datos.get("id");
